...
def test_common_template_config(jmb: Jembe):
    class LR(Component):
        class Config(Component.Config):
            def __init__(
                self,
                ...
                if template is None and base_template is not None:
                    template = (self.default_template_name, base_template)
                super().__init__(
                    template=template,
                    components=components,
                    inject_into_components=inject_into_components,
           ...
           def parent_template(self):
               super(self.__class__, self).parent_template

        pass

    @config(LR.Config(template=("", "common/lrs.html")))
    class LRS(LR):
        pass

    @config(LR.Config(template=("", "common/lri.html"), base_template="common/lri.html"))
    class LRI(LR):
        pass

    @config(LRS.Config(template=("", "common/lrsm.html")))
    class LRSM(LRS):
        pass
    ...
    @jmb.page(
        "p",
        Component.Config(
            components=dict(lrsm=LRSM, lri=LRI, a=(LRS, LRS.Config(template="a.html")))
        ),
    )
    class Page(Component):
        pass

    assert jmb.components_configs["/p/lrsm"].parent_template == (
        "common/lrs.html",
    )
    assert jmb.components_configs["/p/lri"].parent_template == (
        "common/lr.html",
    )
    assert jmb.components_configs["/p/a"].parent_template == (
        "common/lrs.html",
    )
