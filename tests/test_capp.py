...
def test_capp(jmb, client):
    ...
    class TaskList(Component):
        ...
        @classmethod
        def inject_into_components(
            cls, self: Component, component_config: ComponentConfig
        ):
            result = dict()
    ...
    assert r.data == (
        ...
        "\n"
        """<html jmb:name="/tasks" jmb:data=\'{"changes_url":true,"state":{},"url":"/tasks"}\'>"""
        """<head><title jmb:name="/tasks/page_title" jmb:data=\'{"changes_url":false,"state":{"title":"Tasks"},"url":"/wiki/page_title"}\'>Tasks</title></head>"""
        """<body>"""
