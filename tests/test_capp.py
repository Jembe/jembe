...
def test_unauthorisied(jmb, client):
    ...
    assert r.data == (
        ...
        "\n"
        """<html jmb:name="/tasks" jmb:data=\'{"changes_url":true,"state":{},"url":"/tasks"}\'>"""
        """<head><title jmb:name="/tasks/page_title" jmb:data=\'{"changes_url":false,"state":{"title":""},"url":"/tasks/page_title"}\'></title></head>"""
        """<body>"""
...
def test_empty_list(jmb, client):
    ...
    assert r.data == (
        ...
        "\n"
        """<html jmb:name="/tasks" jmb:data=\'{"changes_url":true,"state":{},"url":"/tasks"}\'>"""
        """<head><title jmb:name="/tasks/page_title" jmb:data=\'{"changes_url":false,"state":{"title":"Tasks"},"url":"/tasks/page_title"}\'>Tasks</title></head>"""
        """<body>"""
        """<div jmb:name="/tasks/tasks" jmb:data=\'{"changes_url":true,"state":{"mode":"list","parent_task_id":null,"wip_id":null},"url":"/tasks/tasks"}\'>"""
...
def test_refresh_task_list_x(jmb, client):
    ...
    assert json_response[0]["dom"] == (
        ...
        "<tr>"
        "<td>"
        """<a href="/tasks/tasks/view/1" onclick="$jmb.component('view',task_id=1)">Task 1</a>"""
        "</td>"
        "<td>"
        """<a href="/tasks/tasks/edit/1" onclick="$jmb.component('edit',task_id=1)">edit</a>"""
        """<a href="/tasks/tasks/delete/1" onclick="$jmb.component('delete',task_id=1)">delete</a>"""
        "</td>"
        "</tr>"
        "<tr>"
        "<td>"
        """<a href="/tasks/tasks/view/2" onclick="$jmb.component('view',task_id=2)">Task 2</a>"""
        "</td>"
        "<td>"
        """<a href="/tasks/tasks/edit/2" onclick="$jmb.component('edit',task_id=2)">edit</a>"""
        """<a href="/tasks/tasks/delete/2" onclick="$jmb.component('delete',task_id=2)">delete</a>"""
        "</td>"
        "</tr>"
        "</table>"
        "</div>")
