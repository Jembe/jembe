...
def test_simple_page(jmb, client):
    ...
    assert b'<html lang="en" jmb:name="/simple_page" jmb:data=\'{"changesUrl":true,"state":{},"url":"/simple_page"}\'' in r.data
...
def test_empty_page(jmb, client):
    ...
    r = client.get("/page")
    assert r.status_code == 200
    assert b'<html jmb:name="/page" jmb:data=\'{"changesUrl":true,"state":{},"url":"/page"}\'>' in r.data
...
def test_counter_on_page(jmb, client):
    ...
    assert b'jmb:name="/cop" jmb:data=\'{"changesUrl":true,"state":{"counter":0},"url":"/cop"}\'' in res.data
